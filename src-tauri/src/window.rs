...
use crate::APP;
use log::{info, warn};
use tauri::{Emitter, Listener, Manager, Monitor, WebviewUrl, WebviewWindowBuilder};

fn get_daemon_window() -> tauri::WebviewWindow {
...
fn build_window(label: &str, title: &str) -> (tauri::WebviewWindow, bool) {
...
pub fn config_window() {
    ...
    window.set_min_size(Some(tauri::LogicalSize::new(800.0, 400.0))).unwrap();
    window.set_size(tauri::LogicalSize::new(800.0, 600.0)).unwrap();
...
fn translate_window() -> tauri::WebviewWindow {
    ...
    window.set_size(tauri::PhysicalSize::new(
        (width as f64 * dpi) as u32,
        (height as f64 * dpi) as u32,
    )).unwrap();
...
pub fn image_translate() {
    let app_handle = APP.get().unwrap();
    let state: tauri::State<StringWrapper> = app_handle.state();
    state.0.lock().unwrap().replace_range(.., "[IMAGE_TRANSLATE]");
    let window = translate_window();
...
pub fn updater_window() {
    ...
    window.set_min_size(Some(tauri::LogicalSize::new(600.0, 400.0))).unwrap();
    window.set_size(tauri::LogicalSize::new(600.0, 400.0)).unwrap();
    window.center().unwrap();
}
